...
reviewTitles.forEach(setData);

document.getElementById("data").innerHTML = tableData;

function setData(item, index) {
      tableData += "<table class='" + ((reviewType[index] === "P") ? "prime" : (reviewType[index] === "N") ? "new" : "") + "'>" +
            "<caption>" + item + "</caption>" +
            "<tr>" +
            "<th>By</th>" +
            "<td>" + reviewers[index] + "</td>" +
            "</tr>" +
            "<tr>" +
            "<th>Review Date</th>" +
            "<td>" + reviewDates[index] + "</td>" +
            "</tr>" +
            "<tr>" +
            "<th>Rating</th>" +
            "<td>" + setRating(stars[index]) + "</td>" +
            "</tr>" +
            "<tr>" +
            "<td colspan=2'>" + reviews[index] + "" +
            "</td>" +
            "</tr>" +
            "</table>";
}
